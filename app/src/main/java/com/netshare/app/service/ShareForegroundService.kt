package com.netshare.app.service
...
import com.netshare.app.EasyShareApp
import com.netshare.app.MainActivity
import com.netshare.app.R
...
class ShareForegroundService : Service() {
    ...
    private fun buildNotification(sending: Boolean, progressPermille: Int, fileName: String?): Notification {
        ...
        return NotificationCompat.Builder(this, EasyShareApp.TRANSFER_CHANNEL_ID)
            ...
            .setContentText(fileName ?: "Netshare transfer")
    ...
    companion object {
        ...
        const val ACTION_STOP = "com.netshare.app.STOP_SHARE"
        const val EXTRA_SENDING = "sending"
        const val EXTRA_PROGRESS = "progress"
        const val EXTRA_FILE_NAME = "file_name"

        fun start(context: Context, sending: Boolean, progressPermille: Int = 0, fileName: String? = null) {
            val app = context.applicationContext
            val intent = Intent(app, ShareForegroundService::class.java)
                .putExtra(EXTRA_SENDING, sending)
                .putExtra(EXTRA_PROGRESS, progressPermille)
                .putExtra(EXTRA_FILE_NAME, fileName)
            ContextCompat.startForegroundService(app, intent)
        }

        fun update(context: Context, sending: Boolean, progressPermille: Int, fileName: String?) {
            start(context, sending, progressPermille, fileName)
        }

        fun stop(context: Context) {
            val app = context.applicationContext
            app.startService(
                Intent(app, ShareForegroundService::class.java).setAction(ACTION_STOP)
            )
        }
    }
}
