...
object SignalingCrypto {
    ...
    fun authKey(normalizedCode: String): ByteArray =
        sessionKeys(normalizedCode).auth

    fun encKey(normalizedCode: String): ByteArray =
        sessionKeys(normalizedCode).enc

    /** One PBKDF2 pass → domain-separated auth + enc keys. */
    fun sessionKeys(normalizedCode: String): SessionKeys {
        val master = masterKey(normalizedCode)
        return SessionKeys(
            auth = expandKey(master, "easyshare-v1-auth"),
            enc = expandKey(master, "easyshare-v1-enc")
        )
    }

    data class SessionKeys(val auth: ByteArray, val enc: ByteArray)

    /**
     * Short human check-phrase both devices show after handshake so users can
     * confirm they paired with the intended peer (not another code guesser).
     */
    fun confirmPhrase(normalizedCode: String): String {
        val digest = MessageDigest.getInstance("SHA-256")
            .digest("easyshare-v1-confirm|$normalizedCode".toByteArray(utf8))
        val a = WORDS[(digest[0].toInt() and 0xff) % WORDS.size]
        val b = WORDS[(digest[1].toInt() and 0xff) % WORDS.size]
        return "$a · $b"
    }
